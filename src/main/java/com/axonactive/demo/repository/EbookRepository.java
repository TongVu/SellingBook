...
public interface EbookRepository extends JpaRepository<Ebook, Integer> {
    @Query("SELECT new com.axonactive.demo.service.dto.ebookDto.EbookInfoCategoryAuthorDto(e.title, e.page, e.rating, e.introduction, e.purchased, p.name, p.email, c.name, concat(concat(a.firstName, ' '), a.lastName) as authorFullName) " +
